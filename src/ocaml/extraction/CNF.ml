...
(** val num_vars : cnf -> coq_N **)

let num_vars cs =
  PS.fold (fun _ n -> N.add n (Npos Coq_xH)) (vs_of_cnf cs) N0

(** val max_var_of_clause : literal list -> positive **)

let max_var_of_clause c =
  foldl (fun m l -> Pos.max m (var_of_lit l)) var_tt c

(** val max_var_of_cnf : literal list list -> positive **)

let max_var_of_cnf cs =
  foldl (fun m c -> Pos.max m (max_var_of_clause c)) var_tt cs

(** val num_clauses : cnf -> coq_N **)

let num_clauses cs =
  foldl (fun n _ -> N.add n (Npos Coq_xH)) N0 cs
