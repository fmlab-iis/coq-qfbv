...
let bit_blast_shl_int1 g ls =
  ((g, []), (dropmsl (joinlsl lit_ff ls)))

(** val bit_blast_shl_int :
...
let rec bit_blast_shl_rec g ls ns i =
  match ns with
  | [] -> ((g, []), ls)
  | ns_hd :: ns_tl ->
    let (p, ls_tl) =
      bit_blast_shl_rec g ls ns_tl (N.mul i (Npos (Coq_xO Coq_xH)))
    in
    let (g_tl, cs_tl) = p in
    if eq_op lit_eqType ns_hd (Obj.magic lit_tt)
    then let (p0, ls_hd) = bit_blast_shl_int g_tl ls_tl i in
         let (g_hd, cs_hd) = p0 in ((g_hd, (catrev cs_tl cs_hd)), ls_hd)
    else if eq_op lit_eqType ns_hd (Obj.magic lit_ff)
         then ((g_tl, cs_tl), ls_tl)
         else let (p0, ls_hd) = bit_blast_shl_int g_tl ls_tl i in
...
let bit_blast_shl g ls ns =
  ...
  then let (p, zero_hi) =
       ...
       let (p4, ls_shl) =
         bit_blast_shl_rec g_eq ls (Obj.magic ns_lo) (Npos Coq_xH)
       in
       let (g_shl, cs_shl) = p4 in
       let (p5, ls_ite) = bit_blast_ite g_shl l_eq ls_shl zero in
       let (g_ite, cs_ite) = p5 in
       ((g_ite,
       (catrev cs_zero_hi
         (catrev cs_zero
           (catrev cs_hi (catrev cs_lo (catrev cs_eq (catrev cs_shl cs_ite))))))),
       ls_ite)
  else bit_blast_shl_rec g ls (Obj.magic ns) (Npos Coq_xH)
