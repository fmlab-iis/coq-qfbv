...
let rec bit_blast_mul_rec g ls1 ls2 i =
  ...
  | ls2_hd :: ls2_tl ->
    let (p, lrs_tl) = bit_blast_mul_rec g ls1 ls2_tl (N.add i (Npos Coq_xH))
    in
    let (g_tl, cs_tl) = p in
    if eq_op lit_eqType ls2_hd (Obj.magic lit_tt)
    then let (p0, lrs_hd) = bit_blast_shl_int g_tl ls1 i in
         let (g_hd, cs_hd) = p0 in
         let (p1, lrs_add) = bit_blast_add g_hd lrs_tl lrs_hd in
         ...
         else let (p0, lrs_hd) = bit_blast_shl_int g_tl ls1 i in
              let (g_hd, cs_hd) = p0 in
              let (p1, lrs_and) =
...
let bit_blast_mul g ls1 ls2 =
  bit_blast_mul_rec g ls1 ls2 N0
