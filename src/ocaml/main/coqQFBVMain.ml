...
let args = [
    ...
    ("-grat-log", String (fun str -> option_grat_log_file := Some str), "FILE\tOutput log of GRAT to a specified file");
    ("-gratchk", String (fun str -> option_gratchk_path := str), "PATH\tSpecify path to gratchk (default is " ^ !option_gratchk_path ^ ")");
    ("-gratgen", String (fun str -> option_gratgen_path := str), "PATH\tSpecify path to gratgen (default is " ^ !option_gratgen_path ^ ")");
    ...
    ("-sat-log", String (fun str -> option_sat_log_file := Some str), "FILE\tOutput log of SAT solver to a specified file");
