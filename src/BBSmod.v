...
Definition smodB bs1 bs2 : bits :=
  ...
  if (msb bs1 == msb bs2) || (r_sdiv == (zeros (size bs1))) then
    r_sdiv
...
Definition bit_blast_smod g ls1 ls2 : generator * cnf * word :=
  ...
  let '(g_and, cs_and, r_eq) := bit_blast_eq g_sdiv r_sdiv (copy (size r_sdiv) lit_ff) in
  if (ls1_sign == ls2_sign) || (r_eq == lit_tt) then
    (g_sdiv, cs_sdiv, r_sdiv)
  else
    let '(g_add, cs_add, lrs_add) := bit_blast_add g r_sdiv ls2 in
