...
From BitBlasting Require Import Var QFBV CNF BBCommon BBIte.
...
Fixpoint bit_blast_lshr_int g ls n : generator * cnf * word :=
  match n with
  | O => (g, [::], ls)
...
Fixpoint mk_env_lshr_int E g ls n : env * generator * cnf * word :=
  match n with
  | O => (E, g, [::], ls)
...
Proof.
  ...
  - rewrite /bit_blast_lshr_int /= .
    case => _ _ <- // .
  - rewrite /bit_blast_lshr_int (lock interp_cnf) /=
            -!lock -/bit_blast_lshr_int .
    case Hm: (bit_blast_lshr_int g ls n) => [[g_m cs_m] ls_m] .
    case => _ <- <- .
    move => Hlsbs Hcsm .
    ...
    rewrite enc_bits_joinmsb Hls_mbs (add_prelude_enc_bit_ff Hcsm) // .
Qed .
...
Proof .
  ...
  - by case => _ _ <- .
  - move: IH; rewrite -/bit_blast_lshr_int => IH .
    dcase (bit_blast_lshr_int g ls n) => [[[g_m cs_m] ls_m] Hrec] .
    case => _ _ <- .
...
Fixpoint bit_blast_lshr_rec g ls ns i : generator * cnf * word :=
  match ns with
  | [::] => (g, [::], ls)
  | ns_hd::ns_tl =>
...
Proof .
  ...
  - by case => _ <- <- .
  - rewrite /= .
    dcase (bit_blast_lshr_rec g ls lns_tl (2*i)) => [[[g_tl cs_tl] ls_tl] Hshrrec] .
...
Proof .
  ...
  - rewrite /mk_env_lshr_int /bit_blast_lshr_int /= .
    by case => _ <- <- <- .
  - rewrite /mk_env_lshr_int /= -/mk_env_lshr_int .
    dcase (mk_env_lshr_int E g ls i) => [[[[E_m g_m] cs_m] ls_m] Henv] .
...
Lemma size_mk_env_lshr_int E g n ls E' g' cs lrs :
  ...
  size ls = size lrs .
Proof .
  rewrite /mk_env_lshr_int /= .
  elim : n E' g' cs ls lrs => [|n IH] E' g' cs ls lrs .
  - by case => _ _ _ <- .
  - move: IH; rewrite -/mk_env_lshr_int => IH .
    dcase (mk_env_lshr_int E g ls n) => [[[[E_m g_m] cs_m] ls_m] Hrec] .
    case => _ _ _ <- .
...
Fixpoint mk_env_lshr_rec E g ls ns i : env * generator * cnf * word :=
  match ns with
  | [::] => (E, g, [::], ls)
  | ns_hd::ns_tl =>
...
Proof .
  ...
  - rewrite /mk_env_lshr_rec /= -/mk_env_lshr_rec .
    by case => _ <- <- <- .
  - rewrite /mk_env_lshr_rec /= -/mk_env_lshr_rec .
    dcase (mk_env_lshr_rec E g ls ns_tl (2*i)) => [[[[E_tl g_tl] cs_tl] ls_tl] Hrec] .
...
Proof .
  ...
  - rewrite /mk_env_lshr_int /=; by t_auto_newer .
  - rewrite /mk_env_lshr_int /= -/mk_env_lshr_int .
    dcase (mk_env_lshr_int E g ls n) => [[[[E_m g_m] cs_m] ls_m] Hint] .
...
Proof .
  ...
  - rewrite /mk_env_lshr_int /= -/mk_env_lshr_int .
    ...
    move : (mk_env_lshr_int_newer_gen Hint) => Hggm .
...
Proof .
  ...
  - rewrite /mk_env_lshr_int /=; by case => _ <- <- _ _ .
  - rewrite /mk_env_lshr_int /= -/mk_env_lshr_int .
    dcase (mk_env_lshr_int E g ls n) => [[[[E_m g_m] cs_m] ls_m] Hint] .
    case => _ <- <- _ Hgls .
  ...
  - rewrite /mk_env_lshr_rec /= -/mk_env_lshr_rec .
    ...
    + case : (ns_hd == lit_ff) .
      ...
      * case => _ <- <- _ .
        ...
        move : (mk_env_lshr_int_newer_res (newer_than_lit_le_newer Hgtt Hggtl) Hgtllstl Hint) => Hghdlshd .
        move : (size_mk_env_lshr_int Hint) => -/eqP Hlstllshd .
